...
logger = logging.getLogger("agents.new_teacher_script_4")

S1_GREETING = "Hello Sir, {agent_name} this side from Universal Books. क्या मै {caller_name} sir से बात कर रही हूँ?"
S2_INTRO = (
    "हम test preparation books publish करते हैं for CBSE, JEE और NEET, और customized branded material भी provide करते हैं। "
    "Sir, ये sales call नहीं है, बस ये check करना था कि क्या आप sample books मे interested होंगे?"
)
S3_ASK_CLASSES = "Sir, institute मे main focus कौन सी classes और exams पर है? six to eight, nine ten या eleven twelve? NEET और JEE?"
S4_WHATSAPP_USP = (
    "Sir, मै आपको sample भेजती हूँ — देखने के बाद अगर relevant लगे तो we can discuss further."
)
...
class Step2_Intro(BaseUBAgent):
    ...
    async def unclear_response(self, context: RunCtx, what_they_said: str = "unclear") -> "Step2_Intro":
        await self.say_script("जी सर, मतलब specifically कौन सी classes चलती हैं? six to eight, nine ten या eleven twelve?")
        return Step2_ListenClasses()
    @function_tool
    async def person_busy(self, context: RunCtx, response: str = "busy") -> BaseUBAgent:
        await self.say_script(S_BUSY)
