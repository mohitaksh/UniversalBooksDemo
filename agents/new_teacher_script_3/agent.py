...
logger = logging.getLogger("agents.new_teacher_script_3")

S1_GREETING = "नमस्ते Sir, मै {agent_name} बोल रही हूँ Universal Books से।"
S2_INTRO = (
    "हम CBSE, NEET और JEE के लिए exam-focused books और customized study material publish करते हैं। "
    "Sir call आपको samples के लिए किया है... अभी आप अपने students को market books recommend करते हैं या अपने institute का branded material चलाते हैं?"
)
S3_ASK_CLASSES = "Sir, currently आपके students boards के लिए prepare कर रहे हैं या competitive exams के लिए?"
S4_WHATSAPP_USP = (
    "Sir, मै two three sample chapters WhatsApp और email कर देती हूँ — आप आराम से देख लीजिये। "
    "Sir हमारा content बाकी publishers से थोड़ा अलग है।"
)
...
class Step1b_ConfirmIdentity(BaseUBAgent):
    ...
    @function_tool
    async def identity_confirmed(self, context: RunCtx, response: str = "confirmed") -> "Step2_Intro": return Step2_ListenClasses()
    @function_tool
...
class Step2_Intro(BaseUBAgent):
    ...
    async def unclear_response(self, context: RunCtx, what_they_said: str = "unclear") -> "Step2_Intro":
        await self.say_script("जी सर, मतलब specifically कौन सी classes चलती हैं? boards या competitive exams?")
        return Step2_ListenClasses()
    @function_tool
    async def person_busy(self, context: RunCtx, response: str = "busy") -> BaseUBAgent:
        await self.say_script(S_BUSY)
